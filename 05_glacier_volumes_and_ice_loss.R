####################################################################################################
#######       Obtain the total volumes of glaciers and their volume loss between       #############
#######                    2000 and 2019 in 100-m elevation bins                       #############
#######                                                                                #############
#######                                  by Georg Veh                                  #############
#######                                  12 Oct, 2021                                  #############
#######                      more comments added on 09 March, 2021                     #############
####################################################################################################

# This workflow will make extensive use of 
# - Digitized glacier outlines as reported by the RGI Consortium (2017): Randolph Glacier Inventory - 
#   A Dataset of Global Glacier Outlines: Version 6.0: Technical Report, Global Land Ice Measurements  
#   from Space, Colorado, USA. Digital Media. DOI: https://doi.org/10.7265/N5-RGI-60
# - Gridded data on glacier thickness as reported by Farinotti et al. (2019): A consensus estimate 
#   for the ice  thickness distribution of all glacierson Earth. Nature Geoscience, 12(3), 168-173.
# - Gridded data on glacier elevation change between 2000 and 2019 as reported by Hugonnet et al. 
#   (2021): Accelerated global glacier mass loss in the early twenty-first century. Nature, 592(7856), 
#   726-731.
#
# You will be prompted to download the files from these publications in the course of this script.
# Make sure you have some tens of gigabytes of free local hard drive storage on your machine. 
# Furthermore, note that these scripts were executed on a Windows Server environment with 52 core 
# and 256 GB RAM. If necessary, adapt the number of cores to reduce RAM consuption, although this
# might increase computation time.

# Load packages. Use install.packages("packagename") if one or more of these packages are not yet
...
gl.list <- list.files(path = "D:/nrc_user/veh/LW_F/Shapefiles/00_rgi60", # Change to YOUR file path
                      recursive = T, 
                      include.dirs = T, 
                      full.names = T) %>%
  as_tibble() %>% 
...
# Identify all glaciers that are covered by the six study regions.  We use parallel processing,
# and export the 19 RGI regions to the clusters. For each RGI region, we would like to obtain the 
...
# Export R-packages to the cluster, which will be used in the apply loop. Make sure you 
# downloaded these packages before.

clusterEvalQ(cl = cl, c(library("sf"),
                        library("stringr"),
                        library("tidyverse")))

# Export the file list of the glacier shapefiles and the extent of the study regions to the clusters.

clusterExport(cl = cl, list("gl.list", "sr.wgs"))

# Iterate over the list of glaciers, read the glaciers to memory, and find the glaciers in each
# RGI region that overlap with the study regions.
...
# We would now like to know at which altitudes in our study regions glacier ice is stored.
# We subtract the gridded data of the glacier volumes from gridded data of the glacier surface.
# We first obtain a list of the surface DEMs of all glaciers worldwide. Data are available under
# "surface_DEMs_RGI60.zip" at  https://www.research-collection.ethz.ch/handle/20.500.11850/315707. 
...
# Then obtain a list of ice thickness data from the same data source are available under  
# "composite_thickness_RGI60-all_regions.zip" at
...
# Set up the cluster environment. We use 50 cores - adjust to your system specifications.

cl <- makeCluster(50)
registerDoParallel(cl)

# We iterate over all glacier polygons. We generate text files as output that have the glacier 
# volume in 100-m bins between -1,000 and 9,000 m a.s.l. (thus 100 values). 
...
vol.per.bin <- foreach(i = 1: nrow(all.gl.pol),  
                       ...
                       .combine = cbind) %dopar% {
                         ...
                         # Obtain the location of the file containing the associate ice thickness.
                         ...
                         # Obtain the spatial resolution of the ice thickness data. This is not identical for all files.
                         ...
                         # Generate a stack of rasters with three layers: the surface DEM, the ice thickness,
                         # and the glacier bed topography.
                         
                         gl.stack <- stack(surf, th , bed)
                         names(gl.stack) <- c("surf", "thick", "bed")
                         
                         # Reproject the glacier polygon using the projection of the gridded glacier data.
                         
                         if(st_crs(gl.shp) != st_crs(gl.stack)) { 
                           gl.shp <- st_transform(gl.shp, st_crs(gl.stack)) %>%
                                     st_make_valid()}
                         
                         # Obtain the cell values from these three layers. Do some rough filtering of outliers.
                         ...
                         # If the data are pure noise and no data remain after filtering, write NA to disk.
                         ...
                         if (nrow(df) <= 1) {
                           ...
                           write.table(rep(NA, 100), 
                                       file = paste0("D:/nrc_user/veh/LW_F/Ice_vol_per_bin/", all.gl.pol$RGIId[i], ".txt"), 
                                       quote = F, 
                                       row.names = F, 
                         ...
                         # Otherwise, iterate in 100-m steps from -1000 m a.s.l. (ice below sea level at outlet glaciers) 
                         # to the highest peaks on Earth (9000 m a.s.l.). Count the number of cells in each bin and multiply
                         # with the cell resolution.
                         
                         mat <- mapply(FUN = function(x, y, z) {
                           
                           hist(seq(x, y, by = 1), breaks = seq(-1000, 9000, by = 100), plot = F)$count * z * res.th[1] * res.th[2]},
                         ...
                         # Define a location where to store the output, that is the glacier volume in each 100-m bin. 
...
# Bind all binned glacier volume to one large tibble and shorten the column names to
# the ID in the RGI.
...
# Summary statistics: Calculate the glacier volume in each region in 100-m elevation bins. 
...
# Use the dataset by Hugonnet et al. (2021) to calculate the change in glacier volume in 
# each study region. Download the elevation change maps for the period Jan 2000 to Dec 2019 from
...
# We iterate over the list of elevation change maps to identify the maps that intersect with 
# the study regions.
...
# Bind the polygons containing the extents of the dhdt maps to one large multi-polygon.
...
vol.loss.per.bin <- foreach(i = 1:nrow(all.gl.pol), 
                            ...
                            .combine = cbind) %dopar%   {
                              ...
                              # The spatial resolution of the surface DEMs are not the same for all files. However,
                              # the dhdt maps always have a resolution of 100 x 100 m throughout. Therefore,
                              # reproject the surface DEMs to a spatial resolution of 100 m, if necessary.
                              
                              if (res.surf[1] < 100) {
                                
                                surf.agg <- aggregate(surf, fact = 100/ res.surf[1], fun = median)
                              ...
                              if(nrow(loss.dem.ex) > 1) {
                                ...
                                # Reproject to the same UTM zone as the surface DEM, and use
                                # a consistent spatial resolution of 100 x 100 m.
                                ...
                                if (length(loss.dems) == 0) { 
                                  ...
                                  # If there is only No Data in the dhdt tile, write a text file to disk that contains 
                                  # only NA for each elevation bin.
                                  # Change the folder location, if necessary.
                                  
                                  write.table(rep(NA, 100), 
                                              file = paste0("D:/nrc_user/veh/LW_F/Vol_loss_per_bin/", all.gl.pol$RGIId[i], ".txt"), 
                                              quote = F, 
                                              row.names = F, 
                                              col.names =  all.gl.pol$RGIId[i])
                              ...
                              # Loss data are average rates of mass loss in a 20-year period (2000-2019).
                              # Therefore, multiply the elevation change maps with 20.
                              # We subtract the elevation loss from the surface and call this the 'bed',
                              # although this is not the bed in reality, given that ice may still remain
                              # in a given glacier cell.
                              ...
                              # Extract cell values from the stack, and do some raw filtering of outliers
                              # in the mass loss and elevation data.
                              ...
                              # If there is only noise (i.e. no cells remain after filtering), write NA to disk.
                              ...
                              # Otherwise, calculate the elevation loss (negative values) or elevation gain (positive values)
                              ...
                              mat <- mapply(FUN = function(x, y, z) {
                                ...
                                if( x > y ) { # i.e. mass gain
                                  ...
                                  hist(seq(round(y), round(x), by = 1),
                                       breaks = seq(-1000, 9000, by = 100), plot = F)$count * z * 100 * 100
                              ...
                              # Write the text file to disk. Change the output location to YOUR folder.
...
# Similar to workflow on glacier volumes, read the ice loss data from disk using parallel computing.
...
# Set up cluster, export the list of text files, and read the elevation change data in parallel to memory.
...
# Join both the statistics of total glacier volume and glacier volume loss between 2000 and 2019.

full.data <- left_join(elev.stat.glaciers, vol.loss.stat.glaciers) 

# Change to your location.

saveRDS(full.data, file = "U:/R-Scripts_Full_Himalaya/Regional_glacier_and_melt_volumes.rds" )
