...
#######            more comments added on 16 Nov, 2022                  ########
################################################################################
...
# Load the tables of local cumulative elevation change of glaciers impounding 
# lakes between 2000 and 2019. The elevation changes are aggregated by RGI region
# such that one file may contain data on elevation changes for more than one 
# glacier dam. The folder of glacier elevation changes (dhdt) is called dh_pergla_cut. 
# Please contact Romain Hugonnet (romain.hugonnet@legos.obs-mip.fr), if you have 
# any questions on data processing. We use the cumulative mass change, in which 
# the first observation on Jan 01, 2000, is set to zero.
...
# Load the tables with all GLOFs, and those tables with dated flood volumes V0 
# or peak discharge Qp.
...
# Select only dated ice-dammed GLOFs with reported Qp that happened between 2000
# and 2019.
...
# We now iterate over the tables containing the glacier elevation changes for a 
# given RGI region.
...
# Some post-processing of the table, which now contains reported values of 
# Qp and glacier elevation changes.
...
# Use these values to scale the cumulative elevation change and error to mean 
# of zero and unit standard deviation.

scaled.dhdt.qp.df <- dhdt.list.qp %>%
  mutate(mean_dhdt_scaled = (dhdt - dhdt.scaling.qp$dhdt_mean) / 
                             dhdt.scaling.qp$dhdt_sd,
         se_dhdt_scaled = ((dhdt_err_up - dhdt.scaling.qp$dhdt_mean) / 
                             dhdt.scaling.qp$dhdt_sd) - mean_dhdt_scaled)
...
dhdt.qp.brm <- brm(bf(qp_scale ~  mean_dhdt_scaled + (mean_dhdt_scaled | Glacier_and_lake),
                      quantile = 0.5),
...
# Write the hierarchical model of local changes in Qp versus glacier elevation 
# change to disc.
...
plot_trend_dhdt.qp <- preds.sub %>%
  mutate(dhdt = (mean_dhdt_scaled * dhdt.scaling.qp$dhdt_sd) + 
           dhdt.scaling.qp$dhdt_mean,
         qp = (qp * sd(dhdt.list.qp$Peak_discharge_Qp)) + 
           mean(dhdt.list.qp$Peak_discharge_Qp)) %>%
  ggplot(aes(x = dhdt, 
             y = qp)) +
  ...
  labs(x = expression(paste("Cumulative glacier elevation change 2000-2019 [m]")) ,
       y = expression(paste("Peak discharge ", italic(Q)[p], " [",  m^{3}, ']'))) +
  theme_bw() +
...
slopes.dhdt.qp <- dhdt.qp.brm %>%
  spread_draws(b_mean_dhdt_scaled, `r_Glacier_and_lake`[Glacier, param]) %>%
  filter(param == "mean_dhdt_scaled") %>%
  mutate(mean_qp_scaled = b_mean_dhdt_scaled + r_Glacier_and_lake,
         mean_qp = (mean_qp_scaled * sd(scaled.dhdt.qp.df$Peak_discharge_Qp)) / 
           dhdt.scaling.qp$dhdt_sd ) %>%
  ungroup() %>%
...
# Select only dated ice-dammed GLOFs with reported V0 that happened between 2000 
# and 2019.
...
# We now iterate over the tables containing the glacier elevation changes for 
# a given RGI region.

for (i in 1: length(dhdt)) {
  
  # Read the cumulative elevation change for each glacier, aggregated per RGI 
  # region, to memory. 
  ...
  # Add the observed cumulative glacier elevation change (+ error) immediately  
  # before the outburst to the table.
...
# Some post-processing of the table containing values of V0 and glacier elevation 
# change.
...
# Use these values to scale the cumulative elevation change and error to mean of 
# zero and unit standard deviation.

scaled.dhdt.V0.df <- dhdt.list.V0 %>%
  mutate(mean_dhdt_scaled = (dhdt - dhdt.scaling.V0$dhdt_mean) / 
           dhdt.scaling.V0$dhdt_sd,
         se_dhdt_scaled = ((dhdt_err_up - dhdt.scaling.V0$dhdt_mean) / 
                             dhdt.scaling.V0$dhdt_sd) - mean_dhdt_scaled)

# Set weakly informed priors on the slope and intercept. Maintain all other 
# default priors set in brms.
...
dhdt.V0.brm <- brm(bf(V0_scale ~  mean_dhdt_scaled + (mean_dhdt_scaled | Glacier_and_lake),
                      quantile = 0.5),
...
# Write the hierarchical model of local changes in V0 versus glacier elevation 
# change to disk.
...
plot_trend_dhdt.V0 <- preds.sub %>%
  mutate(dhdt = (mean_dhdt_scaled * dhdt.scaling.V0$dhdt_sd) + 
                 dhdt.scaling.V0$dhdt_mean,
         V0 = (V0 * sd(dhdt.list.V0$Mean_Flood_Volume_V0)) + 
                 mean(dhdt.list.V0$Mean_Flood_Volume_V0)) %>%
  ggplot(aes(x = dhdt, 
             y = V0)) +
  facet_wrap(~Glacier_and_lake, 
             scales = "free",
  ...
  geom_linerange(data = scaled.dhdt.V0.df, 
                 aes(y = Mean_Flood_Volume_V0,
                     ...
                     color = rounded_year)) +
  labs(x = expression(paste("Cumulative glacier elevation change 2000-2019 [m]")) ,
       y = expression(paste("Flood volume ", italic(V)[0], 
                            " [", 10^{6}, " ", m^{3}, ']'))) +
  theme_bw() +
  theme( axis.text   = element_text(size = 7),
         axis.text.x = element_text(size = 7),
...
slopes.dhdt.V0 <- dhdt.V0.brm %>%
  spread_draws(b_mean_dhdt_scaled, `r_Glacier_and_lake`[Glacier, param]) %>%
  filter(param == "mean_dhdt_scaled") %>%
  mutate(mean_V0_scaled = b_mean_dhdt_scaled + r_Glacier_and_lake,
         mean_V0 = (mean_V0_scaled * sd(scaled.dhdt.V0.df$Mean_Flood_Volume_V0)) / 
                    dhdt.scaling.V0$dhdt_sd ) %>%
  ungroup() %>%
  mutate(Region = str_sub(Glacier, 1, 8)) %>%
  rename(Glacier_and_lake = Glacier) %>%
...
################################################################################
#######   Plot the location of the investigated glacier dams and        ########
#######   the local trends in V0 and Qp with glacier elevation change   ########

# Read the polygon containing the outline of the study regions to memory.
# Change to your location accordingly.
...
# Combine the locations of glacier dams with repeat GLOFs to one geometry 
# (point features).
...
# Extract the median and 95% HDI of the trends in V0 and Qp with glacier 
# elevation change. To this end, combine both tibbles containing the posterior 
# distributions of the trends. Change the labels of the RGI regions to a more 
# intuitive description of the region. 
...
trends.V0.qp <- bind_rows(
  ...
  labs(x = expression(paste("Trend in ", italic(V)[0], " [" , 10^{6}, 
                            " ", m^{3},  "]", " and ", italic(Q)[p], " [" ,  m^{3}, " ", s^{-1}, "]",
                            "\nwith cumulative change in glacier thickness [m]\nbetween 2000 and 2019")),
       y = "") +
  theme(axis.text   = element_text(size = 7),
        axis.text.x = element_text(size = 7),
        axis.title  = element_text(size = 7),
...
# Save the plot to disk (Figure 5 a, b)
...
# Load the tables of local mean annual elevation change of glaciers impounding  
# lakes between 2000 and 2019. The elevation changes are aggregated by RGI region
# such that one file may contain data on elevation changes from more
# than one glacier dam. The folder of glacier elevation changes  
# is called dh_pergla_cut. Please contant Romain Hugonnet  
# (romain.hugonnet@legos.obs-mip.fr), if you have any questions on data processing.

rates <- list.files(pattern = "rates.csv$",
                    path = "dh_pergla_cut",
                    full.names = T)

# For each table, extract only the average calculated for the period from 
# Jan 01, 2000, to Jan 01, 2020.
...
for (i in 1: length(rates)) {
  ...
  # Convert to a point feature using the columns 'lat' and 'lon', the
  # centroid of the glacier.
...
# Extract the cumulative elevation change for these glaciers from the tables 
# provided by Romain Hugonnet.
...
ggsave(plot = elev.ch.plot, 
       filename = "elev_change_per_glacier.tiff",
       width = 180,
       height = 140,
       dpi = 280,
       unit = "mm")
