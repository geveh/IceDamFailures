...
# Extract all dated GLOFs. Dates are given as YYYY-MM-DD, so the date vector must have 
# 10 characters. Only use GLOFs that occurred in the period 1900-2021. Also rescale
# the dates to range between -Pi and + Pi to match the parametrization of the von Mises 
# distribution in brms. 
...
# Note that both the predictor and the response were transformed, so
# we need to re-transform the predictions to the original scale
...
for(reg in 1: length(uni.reg)) {
  ...
  doy_post_trends <- doy.list.brm[[reg]] %>%
    add_epred_draws(newdata = data.frame(
      year_scale = (seq(1900, 2021, length.out = 51) - mean(doy.data$year)) / sd(doy.data$year)),
      value = "doy_rescale",
      ndraws = 1000) %>%
...
# Obtain number of dated GLOFs per region.
...
# Calculate the change in annual GLOF timing, that is the shift in days within the year 
# between 1900 and 2021 for all regions. Extract the regional posterior distribution of 
# doy from 1900, and subtract it from the posterior distribution of doy in 2021.
...
doy_change_plot <- bind_rows(doy_post_change.list) %>%
  group_by(region) %>%
  filter(doy_change > quantile(doy_change, 0.005) & doy_change < quantile(doy_change, 0.997)) %>%
  ungroup() %>%
...
bind_rows(doy_post_change.list) %>% 
  group_by(region) %>%
  filter(doy_change > quantile(doy_change, 0.005) & doy_change < quantile(doy_change, 0.995)) %>%
...
for(gl in 1:length(uni.gl)) {
  ...
  trend.doy <- brm(doy_rescale ~ year_scale,
                   data = doy.data.per.glacier %>% filter(Glacier_and_lake == uni.gl[gl]) ,
                   family = von_mises(),
                   prior = bprior,
                   warmup = 2000,
                   iter = 6000,
...
# Plot the posterior changes in annual GLOF timing for all glacier lakes with repeat outbursts.
...
# Obtain the posterior 95% highest density interval (HDI) for the posterior change in
# GLOF timing for all glaciers between the first and the last observed GLOF from a given lake. 

# Posterior distribution of doy in the year of the first observed GLOF from a given lake.
...
# Posterior distribution of doy in the year of the last observed GLOF from a given lake.
...
# Show the statistics from the posterior distribution of local changes in GLOF timing.
...
ggsave(filename = "regression_doy_per_lake.pdf",
       plot = plot_trend_year.doy,
       width = 200,
       height = 235,
       units = "mm")

ggsave(filename = "regression_doy_per_lake.png",
       plot = plot_trend_year.doy,
       width = 180,
       height = 235,
       units = "mm")
